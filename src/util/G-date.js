;(function(){
    ...
    function factory(){
		gDate.toDate = function(date){
            ...
            var day = dataMatch[5]? +dataMatch[5] : now.getDate();
            var hour = dataMatch[7]? +dataMatch[7] : 0;
            var minute = dataMatch[8]? +dataMatch[8] : 0;
            var second = dataMatch[10]? +dataMatch[10] : 0;
        ...
        gDate.transMonth = function(baseDate, num){
            baseDate = gDate.toDate(baseDate);
            var curDay = baseDate.getDate();
            var transDate = new Date(+baseDate);
            transDate.setDate(1);
            transDate.setMonth(transDate.getMonth()+num+1);
            transDate.setDate(0);
            var transDay = transDate.getDate();
            if(transDay > curDay){
                transDate.setDate(curDay);
            }
            return transDate;
        };
        gDate.transYear = function(baseDate, num){
            return gDate.transMonth(baseDate, num*12);
        };
        gDate.transNormal = function(baseDate, num, type){
            baseDate = gDate.toDate(baseDate);
            var transDate = new Date(+baseDate);
            switch(type){
                case 'day': transDate.setDate(transDate.getDate()+num);break;
                case 'hour': transDate.setHours(transDate.getHours()+num);break;
                case 'minute': transDate.setMinutes(transDate.getMinutes()+num);break;
                case 'second': transDate.setSeconds(transDate.getSeconds()+num);break;
                default: transDate.setDate(transDate.getDate()+num);break;
            }
            return transDate;
        };
        gDate.transDay = function(baseDate, num){
            return gDate.transNormal(baseDate, num, 'day');
        };
        gDate.transHour = function(baseDate, num){
            return gDate.transNormal(baseDate, num, 'hour');
        };
        gDate.transMinute = function(baseDate, num){
            return gDate.transNormal(baseDate, num, 'minute');
        };
        gDate.transSecond = function(baseDate, num){
            return gDate.transNormal(baseDate, num, 'second');
        };
        window.gDate = gDate;
    }
})();